...
class VideoStreaming(object):
    ...
    def show(self, source):
        """
        Stream video from either webcam or local file
        source: 'webcam' for webcam, or file path for local video files
        """
        ...
        if source == 'webcam':
            ...
            # Try to open the file/URL directly
            cap = cv2.VideoCapture(source)
            print(f"Using file/URL as video source: {source}")
        while True:
            if self._preview:
                if stop_flag:
...
def webcam():
    """Route to start webcam streaming"""
